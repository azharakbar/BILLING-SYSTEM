...
int  scan(char [],char []);
void space_remove ( char expr [] ) ;
double evaluator ( char exp [] ) ;
...
/*	fstream afile("bill_list.bin",ios::in|ios::binary);
	...
	do{
		...
		else if ( (int) choice == 13 || choice == NULL ) flag = 0 ;
		else flag = 1 ;
...
{
	int flag = 1 , i = 0 , j = 0 ;
	char user[100] = "" , pwd_ = NULL , pwd[100] = "" ;
				...
				{	
					...
					if ( go )
					{
						if(ccp>1)
						{
							...
							strcpy ( temp_filename , c.ccode );
							strcat ( temp_filename , " " );
							strcat ( temp_filename , c.name );
							fstream temp_file ;
							temp_file.open(temp_filename , ios::out );
							temp_file.close () ;							
...
TEMP_CUST:
		while(file.read((char*)&c,sizeof(customer)))
		{
			...
			if ( i == 13 && file.read((char*)&c,sizeof(customer)) )
			{
				...
				ch = NULL;
				while( (int)ch!=13 )	{ gotoxy(105,49); ch=_getch(); }
				goto TEMPLATE_CUST;
			}
		}
...
TEMP_PROD:
	...
	while((int)ch!=13 )	{ gotoxy(107,49); ch=_getch(); }	
	return ;
...
void search_customer( char purpose [] , float* val , int inv_no )
{
	...
	char ch = NULL , cust_name[50] = "" , temp[150] = "" , msg[150] = "" ;
	fstream mainfile , newfile ;
	...
	if ( flag && !strcmpi ( purpose , "SPECIFIC" ) )
	{
		...
		msg_box("|| ** CLOSE THE NOTEPAD FILE TO CONTINUE ** ||");

		char temp[200] = "" ;
		char t[200] = "attrib +R ";
		strcpy ( temp , "\"" );
		strcat ( temp , c_found.ccode );
		strcat ( temp , " " );
		strcat ( temp , c_found.name );
		strcat ( temp , ".txt\"" );
		strcat ( t , temp );
		system ( t ) ;
		if ( system ( temp ) ) c_found.show_data("SINGLE");
		else ; 
...
VIEW_F_F:
	...
	while ( file.read((char*)&b , sizeof(bill)) )
	{
		...
		if ( i == 6  )
		{
			...
			ch = NULL;
			while((int)ch!=13&&(int)ch!=48&&(int)ch!=9) ch = _getch() ;
	...
	ch = NULL ;
	while((int)ch!=13&&(int)ch!=9)ch=_getch() ;
	if ( (int)ch == 9 ) { file.seekg(-1*sizeof(bill) , ios::end); goto VIEW_L_F ; }
	file.close();
	return ;
...
{
	int  i =0  , j = 0 ;
	char temp_barcode[20]="" , temp_prod_name[50]="" , temp_price[10]="" , temp=NULL ;
	fstream file ;
...
{
	int  i =0  , j = 0 ;
	char temp_name[50]="" , temp_ctc[15]="" , temp_cr[10]="" , temp_ccp[10]="" , temp=NULL ;
	fstream mainfile,newfile ;
	customer c ; 
...
int customer::credit_entry( float* val , int inv_no )
{
	...
	if ( val )
	{
		...
		cout<<msg<<"     "<<endl;
		temp = NULL;
		while( temp != 'Y' && temp != 'y' && temp != 'N' && temp != 'n' ) temp = _getch() ;
...
int customer::credit_pay()
{
	...
	{	
		...
		cout<<endl;gotoxy(0,0);
		temp = _getch() ; while((int)temp!=13) temp = _getch() ;
		return 0 ; 
	}
...
{
	int flag = 0 , i = 0 , j = 0 ;
	char ch = NULL , choice = NULL , temp = NULL , temp_barcode[50] = "" , temp_name[50] = "" , temp_price[10] = "" ;
...
int bill::new_bill ( int inv_no )
{
	...
	float tot = 0 ;
	char ch = NULL , temp = NULL , temp_barcode[20]="" , temp_name[50]="" , temp_qty[8]="" ; 
	fstream mainfile , newfile ;
	product p , p_found ;
...
{
	int i = 0 , j = 0 , flag = 0 ;
	char temp_dis[10]="" , temp_cash[10]="" , temp = NULL , choice =NULL ;

START:
	i = 0 ; j = 0 ; flag = 0 ; temp = NULL ; choice = NULL ;
	header();
	gotoxy(0,0);cout<<endl;
	gotoxy(77 , 10) ;
	cout<<"INVOICE BILLING"<<endl;
